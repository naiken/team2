...
public class MyRenderer implements GLSurfaceView.Renderer {
    ...
    private static final int GAME_INTERVAL = 30;// 制限時間は60秒
    // ハンドラを生成
    ...
    private int waitingTime = 2 * (1000 / Game.TIMER_PERIOD);// 2秒まつ
        ...
        if (!mGameOverFlag) {
            ...
            for (int i = 0; i < TARGET_NUM; i++) {
                if (targets[i].isPointInside(x, y)) {
                    ...
                    float theta = (float) Global.rand.nextInt(360) / 180.0f
                            * (float) Math.PI;// 適当な位置
